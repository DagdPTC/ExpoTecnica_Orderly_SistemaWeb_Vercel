import { API, fetchJSON, setAuthToken, getAuthToken } from "./jsService/apiConfig.js";
...
// === login flow (bind inmediato, sin DOMContentLoaded) ===
const form = document.getElementById("loginForm");
const emailEl = document.getElementById("email");
const passEl  = document.getElementById("password");
const submitBtn = form?.querySelector('button[type="submit"]');

// (opcional) log para confirmar que el script cargó
console.log("[login] inicioSesion.js cargado");
...
async function onSubmit(e) {
  ...
  const correo = g.value;

  submitBtn?.setAttribute("disabled", "true");

  try {
    // logout previo (no pasa nada si falla)
    ...
    // Solo Admin (id 1) o Mesero (id 2)
    await assertAllowedRoleOrThrow(me);

    // Éxito → animación + redirección
    playLoginSuccessFxFull();
    showAlert("success", "¡Bienvenido! Entrando…", { timeout: 1200 });
